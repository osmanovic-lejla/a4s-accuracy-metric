...
## ⚠️ Note on the Submitted ZIP Archive

The ZIP file submitted on Moodle contains a slightly reduced version of this repository.
This was necessary because some large files generated by the original A4S framework (e.g., lcld_v2.csv, lcld_v2_random_forest.onnx, cached datasets, and test artifacts) made the archive exceed Moodle’s upload size limit.

The following non-essential files were removed from the ZIP version:
	•	tests/data/lcld_v2.csv (≈180 MB)
	•	tests/data/lcld_v2_random_forest.onnx (≈390 MB)
	•	Any automatically downloaded CIFAR data
	•	Python cache directories (__pycache__, .pytest_cache)

These files are not required to evaluate or run the Brier Score metric, the implemented tests, or the notebook.
All dataset files used in my project (Breast Cancer & CIFAR-10) are automatically downloaded when running the tests, so the ZIP project remains fully functional.

The GitHub version still includes the complete structure for transparency, but the Moodle ZIP had to be cleaned to stay within size constraints.
